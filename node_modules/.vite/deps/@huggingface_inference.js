...
var HF_HUB_INFERENCE_PROXY_TEMPLATE = `https://router.huggingface.co/{{PROVIDER}}`;
...
var HF_HUB_INFERENCE_PROXY_TEMPLATE2 = `${HF_ROUTER_URL}/{{PROVIDER}}`;
var tasks = null;
var providerConfigs = {
...
async function makeRequestOptions(args, options) {
  ...
  const url = endpointUrl ? chatCompletion2 ? endpointUrl + `/v1/chat/completions` : endpointUrl : providerConfig.makeUrl({
    baseUrl: authMethod !== "provider-key" ? HF_HUB_INFERENCE_PROXY_TEMPLATE2.replace("{{PROVIDER}}", provider) : providerConfig.baseUrl,
    model,
    chatCompletion: chatCompletion2,
    task
  });
...
var snippetTextGeneration = (model, accessToken, provider, providerModelId, opts) => {
  if (model.tags.includes("conversational")) {
    const baseUrl = provider === "hf-inference" ? `https://router.huggingface.co/hf-inference/models/${model.id}/v1/chat/completions` : HF_HUB_INFERENCE_PROXY_TEMPLATE.replace("{{PROVIDER}}", provider) + "/v1/chat/completions";
    const modelId = providerModelId ?? model.id;
    const streaming = (opts == null ? void 0 : opts.streaming) ?? true;
    const exampleMessages = getModelInputSnippet(model);
    const messages = (opts == null ? void 0 : opts.messages) ?? exampleMessages;
    const config = {
      ...(opts == null ? void 0 : opts.temperature) ? { temperature: opts.temperature } : void 0,
      max_tokens: (opts == null ? void 0 : opts.max_tokens) ?? 500,
      ...(opts == null ? void 0 : opts.top_p) ? { top_p: opts.top_p } : void 0
    };
    return [
      {
        client: "curl",
...
function getCurlInferenceSnippet(model, accessToken, provider, providerModelId, opts) {
  var _a;
  return model.pipeline_tag && model.pipeline_tag in curlSnippets ? ((_a = curlSnippets[model.pipeline_tag]) == null ? void 0 : _a.call(curlSnippets, model, accessToken, provider, providerModelId, opts)) ?? [] : [];
}
var python_exports = {};
...
function getPythonInferenceSnippet(model, accessToken, provider, providerModelId, opts) {
  ...
  if (model.tags.includes("conversational")) {
    ...
    const snippets = model.pipeline_tag && model.pipeline_tag in pythonSnippets ? ((_a = pythonSnippets[model.pipeline_tag]) == null ? void 0 : _a.call(pythonSnippets, model, accessToken, provider, providerModelId)) ?? [] : [];
...
function getJsInferenceSnippet(model, accessToken, provider, providerModelId, opts) {
  var _a;
  return model.pipeline_tag && model.pipeline_tag in jsSnippets ? ((_a = jsSnippets[model.pipeline_tag]) == null ? void 0 : _a.call(jsSnippets, model, accessToken, provider, providerModelId, opts)) ?? [] : [];
}
export {
  HfInference,
  HfInferenceEndpoint,
